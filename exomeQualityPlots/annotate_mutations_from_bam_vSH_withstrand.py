...
def get_samples_from_patient(mutfile, conversionfile, patient_ID, projectname, bamPath):
    ...
    for line in data_pat:
        ...
        bamfile = bamPath+'/'+patient_ID+'/'+line.strip().split('\t')[col_lib]+'.'+os.environ["RECAL_BAM_EXT"] +'.bam'
        #bamfile = bamPath+'/'+patient_ID+'/'+line.strip().split('\t')[col_lib]+'.bwa.realigned.rmDups.recal.bam'
        print bamfile

        annotate_mutations_from_bam(mutfile, bamfile, sample, qualFile, N)
        #mutfile = mutfile.split('/')[-1].split('.txt')[0] + '.%sQ.txt'%(sample)
        mutfile = mutfile.split('/')[-1].split('.txt')[0] + '.%dQ.txt'%(N)
...
def annotate_mutations_from_bam(mutfile, bamfile, sample, qualFile, N):
    ## read mutation data
    ...
    ## prepare outfile
    annofileName = mutfile.split('/')[-1].split('.txt')[0] + '.%dQ.txt'%(N)
