...
"$ANNOVAR/annotate_variation.pl" -filter -dbtype 1000g2010nov_all -buildver hg19 "${datafile}.filter.intersect" "$ANNDB" || error "Error on line $LINENO in $PROG"
OK $LINENO
...
"$ANNOVAR/annotate_variation.pl" -filter -dbtype 1000g2011may_all -buildver hg19 "${datafile}.tmp1" "$ANNDB" || error "Error on line $LINENO in $PROG"
OK $LINENO
...
"$ANNOVAR/annotate_variation.pl" -filter -dbtype snp132 -buildver hg19 "${datafile}.tmp2" "$ANNDB" || error "Error on line $LINENO in $PROG"
OK $LINENO
...
"$ANNOVAR/annotate_variation.pl" --geneanno --buildver hg19 --outfile "${datafile}.filter.intersect.anno" "${datafile}.tmp3" "$ANNDB" || error "Error on line $LINENO in $PROG"
OK $LINENO

### clean up AnnoVar exonic data, put into final mutation table format
echo "================= [Annotate] reformat annovar"
"$BIN/pindel_reformat_annovar.py" "${datafile}.filter.intersect.anno" "${datafile}.filter" || error "Error on line $LINENO in $PROG"
OK $LINENO

### annotate with normal coverage
echo "================= [Annotate] annotate with normal coverage"
"$BIN/pindel_annotate_normal_coverage.py" "${datafile}.filter.intersect.anno.muts" "${proj}" || error "Error on line $LINENO in $PROG"
OK $LINENO

## annotate with Kinase & Cosmic & Sanger Cancer Gene
echo "================= [Annotate] annotate with cosmic, kinase, sanger cancer gene list"
"$BIN/annotation_COSMIC.py" "${datafile}.filter.intersect.anno.muts.norm.txt" "$COSMICDATA" > "${datafile}.filter.intersect.anno.muts.tmp1" || error "Error on line $LINENO in $PROG"
OK $LINENO

"$BIN/annotation_KINASE.py" "${datafile}.filter.intersect.anno.muts.tmp1" "$KINASEDATA" > "${datafile}.filter.intersect.anno.muts.tmp2" || error "Error on line $LINENO in $PROG"
OK $LINENO

"$BIN/annotation_CANCER.py"  "${datafile}.filter.intersect.anno.muts.tmp2" "$CANCERDATA" "$CONVERT" >  "${datafile}.filter.intersect.anno.muts.norm.anno.txt" || error "Error on line $LINENO in $PROG"
OK $LINENO
