...
{ time ${JAVA6} -Xmx8g -Djava.io.tmpdir="${TMP}" \
        ...
        --out "${PATIENT}.merged.intervals"; } 2>&1 || error "GATK1_65 Interval creation failed"
...
{ time ${JAVA6} -Xmx8g -Djava.io.tmpdir="${TMP}" \
        ...
        --out "${PATIENT}.merged.realigned.bam"; } 2>&1 || error "GATK1_65 Indel realignment failed"
...
{ time ${JAVA6} -Xmx8g -Djava.io.tmpdir="${TMP}" -jar "${GATK1_65}" \
...
do
        ...
        { time ${SAMTOOLS0_1_18} sort "$i" "${base}.bwa.realigned.rmDups"; } 2>&1 || error "Sorting $base failed"
		  assert_file_exists "${base}.bwa.realigned.rmDups.bam"

        echo -e "\\n[Recal_pass2] Indexing $base..."
        { time ${SAMTOOLS0_1_18} index "${base}.bwa.realigned.rmDups.bam"; } 2>&1 || error "Indexing $base failed"
...
do
        ...
        { time ${SAMTOOLS0_1_18} flagstat "$i" > "${base}.bwa.realigned.rmDups.flagstat"; } 2>&1
